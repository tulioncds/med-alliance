    ...
    **Template:**
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            pessoa = None
            if new_user.profile.tipo_usuario == 'M':
                pessoa = new_user.profile.medico
            elif new_user.profile.tipo_usuario == 'C':
                pessoa = new_user.profile.clinica
            elif new_user.profile.tipo_usuario == 'P':
                pessoa = new_user.profile.paciente
            formsets = get_formsets(request, form_class, pessoa)
            form.save_formsets(formsets, pessoa)
            if success_url is None:
        ...
        form = form_class()
        formsets = get_formsets(request, form_class)
    ...
    for key, value in backend.get_context_data().items():
        context[key] = callable(value) and value() or value
...
def get_formsets(request, form_class, obj=None):
    formsets = []
    if request.POST:
        for index, formset in enumerate(form_class.inlines):
            if obj:
                formsets.append(formset(request.POST, queryset=get_formset_queryset(formset, obj), prefix='fs%s'%index))
            else:
                formsets.append(formset(request.POST, queryset=formset.model.objects.none(), prefix='fs%s'%index))
    else:
        for index, formset in enumerate(form_class.inlines):
            if obj:
                formsets.append(formset(queryset=get_formset_queryset(formset, obj), prefix='fs%s'%index))
            else:
                formsets.append(formset(queryset=formset.model.objects.none(), prefix='fs%s'%index))
    return formsets

def get_formset_queryset(formset, obj):
    return formset.model.objects.filter(pk=obj.pk)
