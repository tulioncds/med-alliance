...
@register.filter
def has_event(calendar, hour):
    return calendar.event_set.filter(start=hour).exists()

@register.filter
def check_day_available(day):
    day_part1 = day.get_time_slot(day.start  + datetime.timedelta(hours=8), 
               day.start  + datetime.timedelta(hours=12))
    day_part2 = day.get_time_slot(day.start  + datetime.timedelta(hours=8), 
               day.start  + datetime.timedelta(hours=12))
    return len(day_part1.get_occurrences()) + len(day_part2.get_occurrences()) < 16
